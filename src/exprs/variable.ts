 ...
 */
 ...
 export default () => cache({
     ...
     /* 当前文件 */
     file(): string {
         return this.document()?.uri.fsPath.replace(/\\/g, "/") || '';
     },
     ...
     /* 当前文件目录 */
     fileDirname(): string {
         return this.file() && path.dirname(this.file()).replace(/\\/g, "/");
     },
     ...
     /* 当前文件相对路径 */
     relativeFile(): string {
         return this.file() && vscode.workspace.asRelativePath(this.file()).replace(/\\/g, "/");
     },
 
     /* 当前文件相对路径去除扩展名 */
     relativeFileNoExtension(): string {
         return this.file() && path.basename(this.relativeFile(), this.fileExtname()).replace(/\\/g, "/");
     },
 
     /* 当前相对目录 */
     relativeFileDirname(): string {
         return this.file() && path.dirname(this.relativeFile()).replace(/\\/g, "/");
     },
     ...
     workspaceFolder(): string {
         ...
         // 返回结果
         return workspaceFolder ? workspaceFolder.uri.fsPath.replace(/\\/g, "/") : '';
     },
 
     /* 当前工作目录文件夹名称 */
     workspaceFolderBasename(): string {
         return this.workspaceFolder() && path.basename(this.workspaceFolder()).replace(/\\/g, "/");
     },
     ...
     commands(): Record<string, any> {
        let akey = Prefix + 'commands'
         return {
             ...this.config().get(akey),
